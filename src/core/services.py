...
class NavigationService:
    ...
    def get_dynamic_keyboard(self, session: Any) -> List[List[str]]:
        ...
        if session.mode == "home" or session.level == "home":
            keyboard = [
                [labels.browse, labels.search],
                [labels.about, labels.report],
                [labels.suggest]
            ]
            ...
            if course:
                if session.section == "week_list":
                    ...
                    # Dynamic category buttons for week
                    if course.week_actions:
                        cat_buttons = [self.repository.categories.get(cat).label 
                                     for cat in course.week_actions 
                                     if self.repository.categories.get(cat)]
                        keyboard = [cat_buttons[i:i+2] for i in range(0, len(cat_buttons), 2)]
                        keyboard.append([labels.back, labels.main_menu])
                else:
                    # Course overview with dynamic categories
                    top_buttons = [labels.overview, labels.by_week]
                    if course.top_level_actions:
                        cat_buttons = [self.repository.categories.get(cat).label 
                                     for cat in course.top_level_actions 
                                     if self.repository.categories.get(cat)]
                        top_buttons.extend(cat_buttons[:2])  # Add first 2 categories
                    keyboard = [top_buttons, [labels.more_files], [labels.back, labels.main_menu]]
    ...
    def _build_screen_text(self, session: Any) -> str:
        ...
        if session.level == "course" and session.course_id:
            ...
            if session.section == "week_list":
                return f"\U0001f5c2 <b>{title} — Weeks</b>\n\nSelect a week to browse materials."
            if session.section == "week_category":
                return f"\U0001f4c5 <b>{title} — Week {session.week_number}</b>\n\nChoose a category."
            if session.section == "more_files":
                return f"\U0001f4c2 <b>{title} — More Files</b>\n\nChoose a category."
            return f"\U0001f4d8 <b>{title}</b>\n\nChoose an action below."
