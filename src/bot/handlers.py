...
from src.core.models import DeliveryScope, RetryRequest, SessionMode, TelegramSession
from src.core.services import ButtonLabels, DeliveryService, NavigationService, SearchService
...
# Create global instance for button labels
labels = ButtonLabels()
...
def register_handlers(
    ...
    async def on_callback(query: types.CallbackQuery, state: FSMContext) -> None:
        ...
        # Enhanced search result callbacks
        if data.startswith("search_result:"):
            result_data = data.split(":", 1)[1]
            parts = result_data.split("|")
            if len(parts) >= 3:
                resource_id, course_id, match_type = parts[0], parts[1], parts[2]
                await handle_search_result_selection(query, resource_id, course_id, match_type)
            return
        
        if data.startswith("more_search_results:"):
            query_parts = data.split(":", 2)
            if len(query_parts) >= 3:
                search_query = query_parts[1]
                offset = int(query_parts[2])
                await handle_more_search_results(query, search_query, offset)
            return
        
        if data in ["new_search", "main_menu"]:
            await query.answer()
            if query.message:
                if data == "new_search":
                    await navigate(query.message, state, "nav:search")
                else:
                    await navigate(query.message, state, "nav:main")
            return
        
        await query.answer()

    async def handle_search_result_selection(query: types.CallbackQuery, resource_id: str, course_id: str, match_type: str) -> None:
        """Handle search result selection"""
        await query.answer("📂 Loading resource...")
        
        try:
            # Get resource details and initiate delivery
            session = await load_session(query.bot.get_current())
            
            # Navigate to the course and deliver the specific resource
            # This would integrate with the delivery system
    ...
    async def handle_more_search_results(query: types.CallbackQuery, search_query: str, offset: int) -> None:
        """Handle pagination for search results"""
        await query.answer("🔍 Loading more results...")
        
        try:
            # Get more search results with offset
            search_result = await search.search(search_query, query.from_user.id, "resources")
    ...
    async def handle_search_mode(message: types.Message, state: FSMContext, text: str) -> None:
        ...
        # Fallback: simple title search on all courses
        try:
            fallback_results = []
            query_lower = text.lower()
            for course in repository.list_all_courses():
                course_resources = repository.get_course_resources(course.id)
            ...
            if fallback_results:
                ...
                await _handle_no_results(message, safe_msg, suggestions)
                
        except Exception as e:
            log.error(f"Fallback search also failed: {e}")
            await message.answer(
                "⚠️ <b>Search temporarily unavailable</b>\n\nPlease try again in a few moments.",
                parse_mode="HTML"
            )
    ...
    async def _display_search_results(message: types.Message, results: list, query: str) -> None:
        ...
        for i, result in enumerate(results[:5]):  # Show first 5 results
            ...
            # Add emoji based on match type and score
            ...
            # Store result data for callback handling
            result_data = f"{result.get('id')}|{result.get('course_id')}|{match_type}"
    ...
    async def _handle_no_results(message: types.Message, safe_msg: str, suggestions: list[str]) -> None:
        ...
        # Build suggestion keyboard if available
        if suggestions:
            try:
