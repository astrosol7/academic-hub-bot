...
from api.models import Base, Institution, Student
...
def seed_students(csv_path: Path) -> None:
    ...
    try:
        ...
        if not inst:
            inst_display = config.institution_slug.upper()  # e.g. 'sit' → 'SIT'
            inst = Institution(slug=config.institution_slug, display_name=inst_display)
            db.add(inst)
            db.commit()
            db.refresh(inst)
            log.info("Created institution %s (%s)", inst.slug, inst.id)
        else:
            inst_display = config.institution_slug.upper()
            if inst.display_name != inst_display:
                inst.display_name = inst_display
                db.commit()
