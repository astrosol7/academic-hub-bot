...
def extract_course_view_urls(html: str, page_url: str, base_netloc: str) -> list[str]:
    """Find course/view.php links. Moodle 4 My courses often injects links only in JS/JSON ΓÇö regex + data-course-id."""
    seen: set[str] = set()
    ordered: list[str] = []
...
def download_pdf(
    ...
    if not first.startswith(b"%PDF"):
        ...
        if low.lstrip().startswith(b"<") or b"<html" in low[:4000]:
            raise ValueError(
                f"Not a PDF (got HTML ΓÇö check login or URL): {url}"
            )
    ...
    while queue:
        ...
        if ("/my/courses.php" in url or "/my/index.php" in url) and not quick:
            ...
            if (
                ...
                logging.warning(
                    "No course links in static HTML ΓÇö Moodle often loads them with JavaScript. "
                    "Retrying with headless ChromeΓÇª"
                )
                try:
                ...
                logging.warning(
                    ...
                    "or pass --course-url with a direct course/view.php?id=ΓÇª link."
                )
